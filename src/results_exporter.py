...
def export_all_results(
    ...
    with open(filepath, "w") as f:
        ...
        if np.isnan(test_r2):
            ...
            f.write("   Result: ML performs WORSE than historical mean (negative test R²).\n")
        else:
            f.write("   Result: ML performs similar to / better than historical mean on average.\n")
        f.write("   Note: This is the average across all 5 factors; factor-level results vary.\n\n")
        ...
        f.write("   Note: coverage varies by factor; interpret 'well-calibrated' with caution.\n")
        ...
        if backtest_results is not None and len(backtest_results) > 0:
            ...
            if "Mkt_RF" not in backtest_results.columns:
                f.write("   Backtest results missing 'Mkt_RF'; cannot compute beta/alpha.\n")
            else:
                ...
                for label, col in series_map:
                    if col in backtest_results.columns:
                        ...
                        alpha_ann = stats["alpha_monthly"] * 12 if not np.isnan(stats["alpha_monthly"]) else np.nan
                        f.write(f"   {label}:\n")
                        f.write(f"     - Periods:      {int(backtest_results[col].dropna().shape[0])}\n")
                        f.write(f"     - Sharpe:       {stats['sharpe']:.3f}\n")
                        f.write(f"     - Mean:         {stats['mean']*100:+.2f}% monthly ({stats['mean']*12*100:+.2f}% ann.)\n")
                        f.write(f"     - Volatility:   {stats['vol']*100:.2f}% monthly ({stats['vol']*np.sqrt(12)*100:.2f}% ann.)\n")
                        f.write(f"     - CAPM beta:    {stats['beta']:.3f}\n")
                        f.write(f"     - CAPM alpha:   {stats['alpha_monthly']*100:+.2f}% monthly ({alpha_ann*100:+.2f}% ann.)\n")
                    else:
                        f.write(f"   {label}: column '{col}' not found in backtest_results.\n")
        ...
        conc_row = comparison_df[comparison_df["Strategy"].str.contains("concentrated", case=False, na=False)].iloc[0]
        ew_row = comparison_df[comparison_df["Strategy"].str.contains("Equal-Weight", na=False)].iloc[0]
        ...
        tol = 0.002  # ~0.2% absolute Sharpe tolerance, avoids overinterpreting tiny diffs

        f.write("\n(A) RMW tilt vs baseline (snapshot):\n")
        f.write(f"    Baseline: Sharpe {baseline_sharpe:.3f}, RMW beta {baseline_rmw:.3f}\n")
        f.write(f"    Tilt:     Sharpe {tilt_sharpe:.3f}, RMW beta {tilt_rmw:.3f}\n")
        if abs(tilt_sharpe - baseline_sharpe) <= tol:
            f.write("    → Result: Sharpe essentially unchanged; tilt increases RMW exposure.\n")
        ...
        f.write(f"    Baseline Sharpe {baseline_sharpe:.3f} vs Concentrated Sharpe {conc_sharpe:.3f}\n")
        f.write(f"    RMW beta increases to {conc_rmw:.3f} but Sharpe declines, consistent with diversification loss.\n")

        f.write("\n(C) Equal-weight vs optimized baseline (snapshot):\n")
        f.write(f"    Equal-Weight Sharpe {ew_sharpe:.3f} vs Baseline Sharpe {baseline_sharpe:.3f}\n")
        ...
        if beta_col is not None:
            ...
            f.write(f"   Highest: {most_beta} (β = {betas_df.loc[most_beta, beta_col]:.2f})\n")
            f.write(f"   Lowest:  {least_beta} (β = {betas_df.loc[least_beta, beta_col]:.2f})\n")
    ...
    try:
        ...
        with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
            ...
            if "Beta" in betas_df.columns and "R²" in betas_df.columns and "Alpha" in betas_df.columns:
                betas_df[["Beta", "R²", "Alpha"]].describe().to_excel(writer, sheet_name="CAPM Beta Summary")
            elif "Beta_MKT" in betas_df.columns:
                ...
                betas_df[available].describe().to_excel(writer, sheet_name="FF5 Beta Summary")
