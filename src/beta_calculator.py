...
def calculate_capm_beta(stock_excess_returns, market_excess_returns, min_obs=24):
    ...
    data = pd.DataFrame(
        {"stock": stock_excess_returns, "market": market_excess_returns}
    ).dropna()
...
def calculate_all_betas(
    ...
    for ticker in stock_excess_returns.columns:
        beta_results = calculate_capm_beta(stock_excess_returns[ticker], market_excess)
...
def plot_beta_distribution(betas_df, save_path="results/beta_distribution.png"):
    ...
    ax1.axvline(
        betas_df["Beta"].median(),
        color="green",
        linestyle="--",
        linewidth=2,
        label=f'Median = {betas_df["Beta"].median():.2f}',
    )
