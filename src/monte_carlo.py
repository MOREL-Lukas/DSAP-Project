...
class HistoricalMeanBaseline:  # Simple baseline predicting historical mean of each factor
    ...
    def predict(
        ...
        return pd.DataFrame(
            {f: self.means[f] for f in self.factor_names}, index=X_test.index
        )
...
class MonteCarloFactorSimulator:  # Monte Carlo simulator for factor returns
    ...
    def fit(self, y_train: pd.DataFrame):
        ...
        for f in self.factor_names:
            ...
            print(
                f"{f:10s}: μ = {mu:+.4f} ({mu * 100:+.2f}%), σ = {sd:.4f} ({sd * 100:.2f}%)"
            )
        print("\n" + "-" * 80 + "\nCorrelation Matrix:\n" + "-" * 80)
    ...
    def _chol(
        ...
        try:
            ...
            print(
                "Warning: Correlation matrix not positive definite. Using uncorrelated simulations."
            )
    ...
    def plot_simulation_distribution(
        ...
        ax[0].axvline(
            med, linestyle="--", linewidth=2, label=f"Median: {med * 100:.2f}%"
        )
        ax[0].set(
        ...
        ax[1].plot(
            np.arange(n_periods), sims.mean(axis=0), linewidth=2, label="Mean Path"
        )
        ...
        for series, lab in zip(pv, labels):
            ax[2].plot(
                np.arange(n_periods), series, linewidth=2, label=f"{lab} percentile"
            )
        ax[2].fill_between(
            np.arange(n_periods), pv[0], pv[-1], alpha=0.2, label="5th-95th range"
        )
        ax[2].axhline(0, linestyle="--", alpha=0.3)
        ax[2].set(
            title=f"{factor} - Confidence Bands [{mode}]",
            xlabel="Period",
            ylabel="Return",
        )
        ax[2].legend()
...
def compare_historical_mean_vs_ml(
        ...
        rows.append(
            dict(
                ...
                RMSE_Improvement_pct=(
                    ((hm_rmse - ml_rmse) / hm_rmse) * 100.0 if hm_rmse != 0 else np.nan
                ),
                Hist_Mean_R2=hm_r2,
                ML_R2=ml_r2,
    ...
    r2_tbl["Winner"] = r2_tbl["R2_Improvement"].apply(
        lambda x: "ML" if x > 0 else ("Hist" if x < 0 else "Tie")
    )
...
def _plot_hist_vs_ml_comparison(df, y_test, hm_pred, ml_pred, dates, save_dir):
    ...
    ax[0].set(
        title="R2 Comparison: Historical Mean vs ML", xlabel="Factor", ylabel="R2 Score"
    )
    ax[0].set_xticks(x)
    ...
    ax[2].set(
        title="ML Improvement over Historical Mean",
        xlabel="Factor",
        ylabel="R2 Improvement (ML - Hist Mean)",
    )
    ax[2].set_xticks(x)
    ax[2].set_xticklabels(df["Factor"])
    ax[2].grid(True, alpha=0.3, axis="y")

    ml_w, hm_w = int((df["R2_Improvement"] > 0).sum()), int(
        (df["R2_Improvement"] < 0).sum()
    )
    if ml_w + hm_w:
        sizes = [ml_w, hm_w]
    ...
    plt.tight_layout()
    plt.savefig(
        f"{save_dir}/hist_mean_vs_ml_comparison.png", dpi=300, bbox_inches="tight"
    )
    print(f"Comparison plot saved to: {save_dir}/hist_mean_vs_ml_comparison.png")
    plt.close()

    _plot_timeseries(
        y_test, hm_pred, ml_pred, dates, save_dir, "hist_mean_vs_ml_timeseries.png"
    )
...
def _plot_timeseries(y_test, baseline_pred, ml_pred, dates, save_dir, filename):
    ...
    for i, f in enumerate(FACTOR_NAMES):
        a = ax[i]
        a.plot(
            dates, y_test[f], "o-", label="Actual", linewidth=2, markersize=4, alpha=0.7
        )
        a.axhline(
            baseline_pred[f].iloc[0],
...
def compare_ml_enhanced_monte_carlo(
    ...
    intervals = mc_simulator.get_prediction_intervals(
        X_test, ml_predictions=ml_pred, confidence_level=0.95
    )
    ...
    print("\nPrediction Accuracy:\n" + "-" * 80)
    print(
        df[["Factor", "ML_R2", "ML_Enhanced_R2", "R2_Difference"]].to_string(
            index=False, float_format=lambda x: f"{x:+.4f}"
        )
    )
    ...
    print(
        df[["Factor", "Coverage_95_pct", "Avg_Interval_Width"]].to_string(
    ...
    print(
        ...
        if 90.0 <= avg_cov <= 100.0
        else (
            "   ⚠ Intervals too narrow (underestimating uncertainty)"
            if avg_cov < 90.0
...
def _plot_ml_enhanced_intervals(y_test, ml_pred, intervals, dates, save_dir):
    ...
    for i, f in enumerate(FACTOR_NAMES):
        ...
        lo, hi = intervals[f"{f}_lower"], intervals[f"{f}_upper"]
        a.fill_between(
            dates, lo, hi, alpha=0.3, label="95% Confidence Interval", zorder=1
        )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{save_dir}/ml_enhanced_mc_intervals.png", dpi=300, bbox_inches="tight"
    )
    print(
        f"ML-Enhanced intervals plot saved to: {save_dir}/ml_enhanced_mc_intervals.png"
    )
    plt.close()
