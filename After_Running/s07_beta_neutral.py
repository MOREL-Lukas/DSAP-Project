...
FACTOR_FILE   = "Output/sp500_ff5_betas_2020_2024.csv"
...
# ------------------------------------------------------------
def load_data():
    if not all(os.path.exists(f) for f in [FACTOR_FILE, PRICE_FILE, RF_FILE]):
        raise FileNotFoundError("Missing one or more required data files.")

    betas = pd.read_csv(FACTOR_FILE)

    # Handle both CAPM and FF5 formats automatically
    if "Beta" in betas.columns:  # CAPM
        betas = betas[["Symbol", "Beta"]].dropna().drop_duplicates("Symbol")
        print("✅ Loaded CAPM single-factor betas.")
    elif "Beta_MKT" in betas.columns:  # FF5
        factor_cols = ["Beta_MKT", "Beta_SMB", "Beta_HML", "Beta_RMW", "Beta_CMA"]
        betas = betas[["Symbol"] + factor_cols].dropna().drop_duplicates("Symbol")
        print("✅ Loaded Fama–French 5-Factor betas (MKT, SMB, HML, RMW, CMA).")
    else:
        raise ValueError("⚠️ Beta file does not contain expected columns.")
...
def construct_portfolio(uni, betas, n_long, n_short, simplrets, rf_monthly_last):
    df = uni.reset_index().merge(betas, on="Symbol", how="inner")

    # Drop rows missing expected return or market beta (handles CAPM and FF5 cases)
    beta_col = "Beta_MKT" if "Beta_MKT" in df.columns else "Beta"
    df = df.dropna(subset=["ExpRet", beta_col])
    # Merge sector info
    ...
    # Use Beta_MKT if available, else fall back to Beta
    beta_col = "Beta_MKT" if "Beta_MKT" in df.columns else "Beta"
    beta_long = (longs["Weight"] * longs[beta_col]).sum()
    beta_short = (shorts["Weight"] * shorts[beta_col]).sum()
    ...
    # --- Multi-factor neutrality adjustment ---
    factor_cols = [c for c in ["Beta_MKT", "Beta_SMB", "Beta_HML", "Beta_RMW", "Beta_CMA"] if c in port.columns]
    if factor_cols:
        for f in factor_cols:
            port[f + "_contrib"] = port["Weight"] * port[f]
        B = port[factor_cols].to_numpy()
        w = port["Weight"].to_numpy().reshape(-1, 1)
        # Compute current exposures and neutralize via least-squares adjustment
        exposures = (B.T @ w)                     # shape (n_factors, 1)
        adjustment = B @ np.linalg.pinv(B.T @ B) @ exposures
        port["Weight"] = port["Weight"] - adjustment.flatten()
        port["Weight"] *= GROSS_EXPOSURE / port["Weight"].abs().sum()
    else:
        # fallback: simple CAPM beta neutrality if FF5 factors not found
        port["Beta_Contribution"] = port["Weight"] * port.get("Beta", 0)
    ...
    beta_p = 0.0
    if factor_cols:
        beta_p = float((B.T @ port["Weight"].to_numpy()).sum())  # residual exposure measure
    return port.sort_values("Weight", ascending=False), exp_excess, exp_nominal, last_excess, beta_p, vol_m, sharpe, float(cash_weight)


def objective(exp_excess, exposures, cash_abs, last_excess=None, vol_m=None):
    exposure_penalty = PEN_BETA * np.sum(np.abs(exposures))
    score = exp_excess - exposure_penalty - LAMBDA_CASH * abs(cash_abs)
...
def print_factor_exposures(port):
    """Print the portfolio's total exposure to each FF5 factor."""
    factor_cols = [c for c in ["Beta_MKT", "Beta_SMB", "Beta_HML", "Beta_RMW", "Beta_CMA"] if c in port.columns]
    if not factor_cols:
        print("⚠️ No FF5 factor columns found in portfolio.")
        return

    exposures = {}
    for f in factor_cols:
        exposures[f] = (port["Weight"] * port[f]).sum()

    print("\n🧮 Portfolio Factor Exposures (should be near 0):")
    for f, val in exposures.items():
        print(f"  {f:<10}: {val:>8.4f}")
...
def main():
    ...
    print(f"Factor-neutral portfolio using Fama–French 5-Factor betas (MKT, SMB, HML, RMW, CMA).")
...
if __name__ == "__main__":
    ...
    # --- Diagnostic: check factor exposures for best portfolio ---
    print_factor_exposures(best["port"])
