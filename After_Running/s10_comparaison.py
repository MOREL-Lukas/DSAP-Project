...
# Try to use the correct beta columns (FF5 or CAPM)
if "Beta" in port.columns:
    beta_cols = ["Beta"]
elif "Beta_MKT" in port.columns:
    beta_cols = ["Beta_MKT", "Beta_SMB", "Beta_HML", "Beta_RMW", "Beta_CMA"]
else:
    beta_cols = []

cols_to_keep = ["Symbol", "Weight", "Side"] + beta_cols
port = port[cols_to_keep]
...
# Compute average beta exposure (MKT or multi-factor)
if "Beta" in port.columns:
    avg_beta = (port["Beta"] * port["SignedWeight"]).sum()
elif "Beta_MKT" in port.columns:
    avg_beta = (port["Beta_MKT"] * port["SignedWeight"]).sum()
else:
    avg_beta = np.nan
...
if mkt_rets is not None:
    ...
    (rets["Portfolio"].mean() / rets["Portfolio"].std()) * np.sqrt(12)
    if rets["Portfolio"].std() > 0 else np.nan
)
