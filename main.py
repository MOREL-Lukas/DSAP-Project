...
def select_overlay_models(results_df: pd.DataFrame, trained_models: dict):
    ...
    if r2_col not in val_df.columns:
        raise KeyError(
            f"results_df missing R2 column. Found columns: {list(val_df.columns)}"
        )
    ...
    for fac in FACTOR_COLS:
        ...
        per_factor_lambda[fac] = (
            0.4 if fac == "RMW" else 0.3 if fac == "Mkt-RF" else 0.2
        )
...
def run_tests():
    ...
    print("-" * 80 + "\n")
    result = subprocess.run(["pytest", "tests/", "-v"], cwd=".", check=False)
    print("\n" + "=" * 80)
...
def main() -> None:
    ...
    tickers = [
        t
        for t in pd.read_csv("data/raw/sp500_tickers.csv", header=None)[0]
        if t != "WBA"
    ]
    classify_sp500_factors(tickers).to_csv(
        "data/processed/sp500_ff5_classifications.csv", index=False
    )
    ...
    overlay_factors, per_factor_model, per_factor_lambda = select_overlay_models(
    ...
    comparison_df = pd.DataFrame(
        [
            calc_portfolio_stats(
                ff5_equal_weight, "data/processed/Fama_French.csv", "Equal-Weight"
            ),
            calc_portfolio_stats(
                ff5_baseline, "data/processed/Fama_French.csv", "Baseline (no tilt)"
            ),
            calc_portfolio_stats(
                ff5_tilt, "data/processed/Fama_French.csv", "RMW Tilt"
            ),
            calc_portfolio_stats(
                ff5_concentrated,
                "data/processed/Fama_French.csv",
    ...
    baseline_sharpe = comparison_df.loc[
        comparison_df["Strategy"] == "Baseline (no tilt)", "Sharpe"
    ].values[0]
    tilt_sharpe = comparison_df.loc[
        comparison_df["Strategy"] == "RMW Tilt", "Sharpe"
    ].values[0]
    conc_sharpe = comparison_df.loc[
        comparison_df["Strategy"] == "RMW-50 (concentrated)", "Sharpe"
    ].values[0]

    baseline_rmw = comparison_df.loc[
        comparison_df["Strategy"] == "Baseline (no tilt)", "Portfolio_Beta_RMW"
    ].values[0]
    tilt_rmw = comparison_df.loc[
        comparison_df["Strategy"] == "RMW Tilt", "Portfolio_Beta_RMW"
    ].values[0]
    conc_rmw = comparison_df.loc[
        comparison_df["Strategy"] == "RMW-50 (concentrated)", "Portfolio_Beta_RMW"
    ].values[0]
    ...
    if tilt_sharpe > baseline_sharpe:
        print(
            f"   → ✓ YES: Tilt improves Sharpe by {(tilt_sharpe/baseline_sharpe - 1)*100:+.1f}%"
        )
    else:
        print(
            f"   → ✗ NO: Tilt reduces Sharpe by {(1 - tilt_sharpe/baseline_sharpe)*100:.1f}%"
        )
    ...
    if conc_sharpe > baseline_sharpe:
        print(
            f"   → ✓ YES: Concentration improves Sharpe by {(conc_sharpe/baseline_sharpe - 1)*100:+.1f}%"
        )
    else:
        print(
            f"   → ✗ NO: Concentration reduces Sharpe by {(1 - conc_sharpe/baseline_sharpe)*100:.1f}%"
        )
