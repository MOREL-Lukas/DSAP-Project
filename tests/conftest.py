...
def synthetic_ml_dataset() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DatetimeIndex]:
    ...
    X = pd.DataFrame(
        np.random.randn(n_months, n_features), index=dates, columns=feature_names
    )
...
def assert_valid_weights(
    weights: np.ndarray, long_only: bool = True, tolerance: float = 1e-6
):
    ...
    assert np.isclose(
        weights.sum(), 1.0, atol=tolerance
...
def assert_valid_covariance(
    cov: np.ndarray, check_positive_definite: bool = True, tolerance: float = 1e-10
):
    ...
    assert np.allclose(cov, cov.T, atol=tolerance), "Covariance must be symmetric"

    if check_positive_definite:
        eigvals = np.linalg.eigvals(cov)
        assert (
            eigvals >= -tolerance
        ).all(), f"Covariance must be positive semi-definite, got min eigval: {eigvals.min()}"
